...
public class CameraTransformsTests {
  ...
  public void testCalculateVisibleCoordinates() {
    ...
    camerapose = new Pose3d(new Translation3d(5, 5.5, 1.3), new Rotation3d(0, 0, Math.PI));
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
    assertEquals(4, res.size());
    // System.out.println(camerapose);
    // System.out.println(res.size() + " tags found");

    camerapose = new Pose3d(new Translation3d(0, 5.5, 1.3), new Rotation3d(0, 0, Math.PI));
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
    assertEquals(0, res.size());
    // System.out.println(camerapose);
    // System.out.println(res.size() + " tags found");

    camerapose = new Pose3d(new Translation3d(8, 5.5, 1.3), new Rotation3d(0, 0, Math.PI / 2));
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
    assertEquals(0, res.size());
    // System.out.println(camerapose);
    // System.out.println(res.size() + " tags found");

    camerapose = new Pose3d(new Translation3d(5, 5, 0), new Rotation3d(0, Math.PI / 20, Math.PI));
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
    assertEquals(4, res.size());
    // System.out.println(camerapose);
    // System.out.println(res.size() + " tags found");

    camerapose = new Pose3d(new Translation3d(4.0, 5.547868, 1.368552), new Rotation3d(0, 0, 0));
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
    // special case since the target point should never leave the dead center of the image frame
    assertEquals(0, res.size());
    ...
    camerapose =
        new Pose3d( // The distance 3.765 was choosen to be just farther than the 20 degree distance
            // from the point
            new Translation3d(3.765 - 0.0381, 5.547868, 0), new Rotation3d(0, 0, Math.PI));
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 40.0);
    assertEquals(4, res.size());
    assertEquals(0, res.get(0).get(0, 0), DELTA);
    assertEquals(
        1, res.get(0).get(1, 0), 0.01); // within 1% of the edge of the image is good enough
            ...
            new Rotation3d(
                ...
                Math.PI
                    + Units.degreesToRadians(
                        39.9)) // Note that this is just barely less than half the HFOV
            );
    res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
    assertEquals(4, res.size());
    assertEquals(
        1, res.get(0).get(0, 0), 0.01); // within 1% of the edge of the image is good enough
    assertEquals(0, res.get(0).get(1, 0), DELTA);
  }
  ...
  public void testCropSpace() {
    ...
    double[] bounds = CameraTransforms.FindBounds(res, margin.getX(), margin.getY());

    assertEquals(4, bounds.length);
    assertEquals(-0.2, bounds[0], DELTA); // llx
    assertEquals(-margin.getY(), bounds[1], DELTA); // lly
    assertEquals(0.2, bounds[2], DELTA); // urx
