...
public class CameraTransformsTests {
  ...
  //     // special case since the target point should never leave the dead center of the image
  // frame
  ...
  //     camerapose = new Pose3d( // The distance 3.765 was choosen to be just farther than the 20
  // degree distance from the point
  ...
  //     assertEquals(4, res.size());
  //     assertEquals(0, res.get(0).get(0, 0), DELTA);
  //     assertEquals(1, res.get(0).get(1, 0), 0.01); // within 1% of the edge of the image is good
  // enough

  //     camerapose = new Pose3d(
  //         new Translation3d(4.0, 5.547868, 1.368552),
  //         new Rotation3d(0,0,Math.PI+Units.degreesToRadians(39.9)) // Note that this is just
  // barely less than half the HFOV
  //     );
  //     res = CameraTransforms.CalculateVisibleCoordinates(camerapose, tag_coords, 80, 56);
  //     assertEquals(4, res.size());
  //     assertEquals(1, res.get(0).get(0, 0), 0.01); // within 1% of the edge of the image is good
  // enough
  //     assertEquals(0, res.get(0).get(1, 0), DELTA);
  // }
