...
public class LookupTableTests {
  ...
  @Test
  public void testLookupTableInverselyRelated() {
    var table =
        new FakeLookupTable()
            .addRows(
                List.of(
                    new FakeTableRow(Meters.of(0), MetersPerSecond.of(100)),
                    new FakeTableRow(Meters.of(10), MetersPerSecond.of(0))));
    ...
    // Check interpolated measure (LaunchSpeed) values
    assertEquals(
        100, getLookupSpeed.apply(Meters.of(-1))); // If less than lowest value, expect lowest value
    assertEquals(
        0,
        getLookupSpeed.apply(Meters.of(11))); // If higher than highest value, expect highest value
    assertEquals(
        100, getLookupSpeed.apply(Meters.of(0))); // If exact value, expect exact value (low)
