...
public class BaseSwerveDrive extends SubsystemBase {
    ...
    public BaseSwerveDrive(
            BaseSwerveModule[] swerveModules, SwerveConfigs swerveConfigs, Supplier<Rotation2d> getRotation) {
        ...
        m_odometry =
                new SwerveDrivePoseEstimator(m_kinematics, getGyroRotation(), getModulePositions(), initialPoseMeters);
        ...
        m_moduleVelocityPIDTuner = new PIDTuner(
        ...
        m_moduleAnglePIDTuner = new PIDTuner(
        ...
        m_logger.addNumber("SwerveDrive/Rotation_deg", isDebugMode, () -> getOdometryRotation()
                .getDegrees());
        m_logger.addNumber("SwerveDrive/Gyro_angle_deg", isDebugMode, () -> getGyroRotation()
                .getDegrees());
    }
    ...
    public void move(ChassisSpeeds chassisSpeeds) {
        chassisSpeeds.vxMetersPerSecond = MathUtil.clamp(chassisSpeeds.vxMetersPerSecond, -1, 1)
        ...
        chassisSpeeds.vyMetersPerSecond = MathUtil.clamp(chassisSpeeds.vyMetersPerSecond, -1, 1)
                * m_swerveConfigs.maxRobotSpeed_mps()
                * TranslationSpeedModifier;
        chassisSpeeds.omegaRadiansPerSecond = MathUtil.clamp(chassisSpeeds.omegaRadiansPerSecond, -1, 1)
    ...
    public void moveFieldRelativeAngle(double xPercentSpeed, double yPercentSpeed, Rotation2d angle, double magnitude) {
        ...
        if (isDefaultAlliance()) {
            ...
            // Rotate the direction if we're on the non default side (if 0,0 is on the blue side and we're red, we need
            // to rotate our perspective)
            speeds = ChassisSpeeds.fromFieldRelativeSpeeds(
                    xPercentSpeed, yPercentSpeed, omega, getOdometryRotation().minus(new Rotation2d(Math.PI)));
        }
        move(speeds);
    }
    ...
    public void moveToTarget(double maxTranslationSpeedPercent) {
        Pose2d pose = getPose();
        double x =
                MathUtil.clamp(m_XPid.calculate(pose.getX()), -maxTranslationSpeedPercent, maxTranslationSpeedPercent);
        double y =
                MathUtil.clamp(m_YPid.calculate(pose.getY()), -maxTranslationSpeedPercent, maxTranslationSpeedPercent);
    ...
    public void updateModuleOnField(BaseSwerveModule swerveModule, Pose2d robotPose) {
        ...
        Transform2d transform =
                new Transform2d(swerveModule.getTranslation().times(5), swerveModule.getModuleState().angle);
