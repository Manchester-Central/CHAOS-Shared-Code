...
public class CameraTransforms {
   ...
   *
   * <p>Generates a column major view matrix.
   *
   ...
   * UNIMPLEMENTED - For Students to complete!
   *
   * <p>Generates a column major frustum-projection matrix.
   *
   * @param n - near plane
   * @param f - far plane
   * @param l - left plane
   ...
   * UNIMPLEMENTED - For Students to complete!
   *
   * <p>Overloaded Function for convenience.
   *
   ...
   * UNIMPLEMENTED - For Students to complete!
   *
   * <p>Calculates each quad's corners in the camera's view.
   *
   * @param pose camera pose, not a robot pose
   * @param quads squares with their model/world transforms already applied
   * @param hfov horizontal field of view (degrees)
  ...
  public static ArrayList<Matrix<N4, N1>> CalculateVisibleCoordinates(Pose3d pose, Quad[] quads, double hfov, double vfov, double n, double f) {
    ...
    for (Quad q : quads) {
      ...
      for (int idx = 0; idx < points.size(); idx++) {
        ...
        if (-1 < p.get(0, 0) && p.get(0, 0) < 1 && 
          -1 < p.get(1, 0) && p.get(1, 0) < 1 &&
          -1 < p.get(2, 0) && p.get(2, 0) < 1)
        {
          final_points.add(p);
        }
      }
   ...
   * UNIMPLEMENTED - For Students to complete!
   *
   * <p>Finds the region of interest that contains all of the listed points.
   *
   * @param points List of vertices that were probably found from CalculateVisibleCoordinates()
   * @param margins horizontal and vertical margins applied to both sides of the region, in camera
   *     relative values [-1,1]
   * @return {ll.x, ll.y, ur.x, ur.y}
   */
  public static double[] FindBounds(ArrayList<Matrix<N4, N1>> points,
